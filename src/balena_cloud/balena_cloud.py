...
class BalenaCloud:
    ...
    @dataclass
    class DeviceServiceVariable:
        """Namespace for handling device service variable related requests."""
        ...
        async def get(self, variable_id: int) -> EnvironmentVariable:
            ...
            if not response["d"]:
                msg = (
                    "No device service environment variable found with the provided ID."
                )
                raise BalenaCloudResourceNotFoundError(msg)
            return EnvironmentVariable.from_dict(response["d"][0])
